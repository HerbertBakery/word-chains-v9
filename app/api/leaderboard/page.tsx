...
type Leader = { userId: string; username?: string | null; image?: string | null; value: number };
    ...
    <div className="mx-auto max-w-5xl p-6 space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500">Click a row to view that player’s stats.</p>
        </div>
          ...
          <tbody>
            ...
            {!loading && !error && rows.length === 0 && (
              <tr><td colSpan={3} className="p-6 text-center text-neutral-500">
                No data yet. Seed one PlayerStats row or play a game that posts to <code className="px-1 py-0.5 rounded bg-neutral-100">/api/stats/ingest</code>.
              </td></tr>
            )}
            {!loading && !error && rows.map((r, idx) => (
              <tr key={r.userId} className="border-t hover:bg-neutral-50 cursor-pointer"
