...
export async function POST(req: Request) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.dexUnlock.upsert({
      where: { userId_category_key: { userId, category: cat, key: canonical } },
      update: {},
      create: { userId, category: cat, key: canonical },
    });
    // delete glow, if present
    await tx.dexDiscovery.deleteMany({
      where: { userId, category: cat, key: canonical },
    });
  });
